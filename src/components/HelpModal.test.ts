...
describe('HelpModal', () => {
  it('renders the §5.6 wording for all three yellow bands when open', () => {
    ...
    expect(screen.getByText(/from the same country/)).toBeInTheDocument();
    expect(screen.getByText(/one that was on sale the year the answer was built/)).toBeInTheDocument();
  ...
  it('explains that only green counts and locks', () => {
    render(HelpModal, { props: { open: true, onclose: vi.fn() } });
    expect(screen.getByText(/Only 🟩 counts towards your score, and only 🟩 locks a field in\./)).toBeInTheDocument();
  });
