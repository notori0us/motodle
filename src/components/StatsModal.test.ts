...
describe('StatsModal', () => {
  ...
  it('always shows all 8 distribution buckets, zeros included, in descending order', () => {
    const { container } = render(StatsModal, {
      props: { open: true, stats: createInitialStats(), highlightScore: null, onclose: vi.fn(), onshare: vi.fn() },
    });
  ...
  it('highlights the bucket matching highlightScore', () => {
    ...
    const { container } = render(StatsModal, {
      props: { open: true, stats, highlightScore: 9, onclose: vi.fn(), onshare: vi.fn() },
    });
  ...
  it('renders played, win%, current and max streak', () => {
    ...
    render(StatsModal, { props: { open: true, stats, highlightScore: null, onclose: vi.fn(), onshare: vi.fn() } });
  ...
  it('calls onshare when the Share button is clicked', async () => {
    const onshare = vi.fn();
    render(StatsModal, {
      props: { open: true, stats: createInitialStats(), highlightScore: null, onclose: vi.fn(), onshare },
    });
